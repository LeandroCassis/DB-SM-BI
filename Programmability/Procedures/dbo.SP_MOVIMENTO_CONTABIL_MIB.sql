...
GO
CREATE PROCEDURE [dbo].[SP_MOVIMENTO_CONTABIL_MIB] @EMPRESA VARCHAR(10), @DATA_INICIAL DATE, @DATA_FINAL AS DATE AS
...
AND LCT.DATLCT BETWEEN CAST(DATEADD(month, DATEDIFF(month, 0, @DATA_INICIAL), 0) AS DATE) AND EOMONTH(@DATA_FINAL)
...
--,IIF(NATCTA='C',1,-1)*ABS(E650SAL.salmes) '#MOVIMENTO CONTÁBIL'
,ABS(E650SAL.salmes)*
...
WHERE E650SAL.MESANO = DATEADD(MONTH,-1,CAST(DATEADD(month, DATEDIFF(month, 0, @DATA_INICIAL), 0) AS DATE))
