...
FROM SAPIENS.SAPIENS.E410COT WITH (NOLOCK)                        -- Tabela principal: Cotações
    ...
    OUTER APPLY (
        SELECT TOP 1 IPO.CODEMP,  IPO.CODFIL, IPO.NUMOCP, IPO.SEQIPO
        FROM SAPIENS.SAPIENS.E420IPO IPO WITH (NOLOCK)
        WHERE E420OCP.CODEMP = IPO.CODEMP
        AND E420OCP.CODFIL = IPO.CODFIL
        AND E420OCP.NUMOCP = IPO.NUMOCP
        AND E410COT.CODPRO = IPO.CODPRO
 --       AND E410COT.PROSER <> 'S'
    ) E420IPO
   ...
   -- ITENS DA NOTA FISCAL DE COMPRA PRODUTO
    ...
    OUTER APPLY (
        SELECT TOP 1 ISO.CODEMP,  ISO.CODFIL, ISO.NUMOCP, ISO.CODSER, ISO.SEQISO
        FROM SAPIENS.SAPIENS.E420ISO ISO WITH (NOLOCK)
        WHERE E420OCP.CODEMP = ISO.CODEMP
        AND E420OCP.CODFIL = ISO.CODFIL
        AND E420OCP.NUMOCP = ISO.NUMOCP
        AND E410COT.CODSER = ISO.CODSER
     --   AND E410COT.PROSER = 'S'
    ) E420ISO
...
FROM [SQLMML].[Sapiens_Prod].[dbo].[E410COT] WITH (NOLOCK)                        -- Tabela principal: Cotações
    ...
    -- ITENS DA ORDEM DE COMPRA PRODUTO
    LEFT OUTER JOIN OPENQUERY(SQLMML, '
        SELECT CODEMP, CODFIL, NUMOCP, CODPRO, SEQIPO
        FROM (
            SELECT CODEMP, CODFIL, NUMOCP, CODPRO, SEQIPO,
                   ROW_NUMBER() OVER (PARTITION BY CODEMP, CODFIL, NUMOCP, CODPRO ORDER BY SEQIPO) AS RN
            FROM Sapiens_Prod.dbo.E420IPO WITH (NOLOCK)
        ) T WHERE RN = 1
    ') E420IPO
   ...
   -- ITENS DA NOTA FISCAL DE COMPRA PRODUTO
    LEFT OUTER JOIN [SQLMML].[Sapiens_Prod].[dbo].[E440IPC] WITH (NOLOCK)
        ON E420IPO.CODEMP = E440IPC.CODEMP
        AND E420IPO.CODFIL = E440IPC.CODFIL
        AND E420IPO.NUMOCP = E440IPC.NUMOCP
        AND E420IPO.SEQIPO = E440IPC.SEQIPO
    ...
    -- ITENS DA ORDEM DE COMPRA SERVIÇO
    LEFT OUTER JOIN OPENQUERY(SQLMML, '
        SELECT CODEMP, CODFIL, NUMOCP, CODSER, SEQISO
        FROM (
            SELECT CODEMP, CODFIL, NUMOCP, CODSER, SEQISO,
                   ROW_NUMBER() OVER (PARTITION BY CODEMP, CODFIL, NUMOCP, CODSER ORDER BY SEQISO) AS RN
            FROM Sapiens_Prod.dbo.E420ISO WITH (NOLOCK)
        ) T WHERE RN = 1
    ') E420ISO
