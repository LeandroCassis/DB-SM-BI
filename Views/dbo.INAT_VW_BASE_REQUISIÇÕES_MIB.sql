...
GO
CREATE   VIEW [dbo].[INAT_VW_BASE_REQUISIÇÕES_MIB] 
AS WITH BaseValores AS (
    ...
    LEFT JOIN Engeman.Engeman.TipPon WITH (NOLOCK) ON PonConAcu.CodTipPon = TipPon.CodTipPon
...
--        SUM(IIF(PonConAcu.CodTipPon = 3, ColAcu.Valor, 0)) AS KILOMETRAGEM_RAW,
--        SUM(IIF(PonConAcu.CodTipPon = 1, ColAcu.Valor, 0)) AS HORIMETRO_RAW
--    FROM Engeman.Engeman.Aplic
--    LEFT JOIN Engeman.Engeman.CenCus ON Aplic.CodCen = CenCus.CodCen
--    LEFT JOIN Engeman.Engeman.ColAcu ON Aplic.CodApl = ColAcu.CodApl
--    JOIN Engeman.Engeman.PonConAcu ON Aplic.CodApl = PonConAcu.CodApl AND PonConAcu.CodPonAcu = ColAcu.CodPonAcu
--    LEFT JOIN Engeman.Engeman.TipPon ON PonConAcu.CodTipPon = TipPon.CodTipPon
--    LEFT JOIN Engeman.Engeman.TIPAPLIC WITH (NOLOCK)
--    ON APLIC.CODTIPAPL = TIPAPLIC.CODTIPAPL
--
--
--   WHERE ColAcu.DatHor >= '01/01/2023'-- AND '31/12/2023'
--   AND (CASE 
--WHEN CHARINDEX('CAMINHÃO',TIPAPLIC.DESCRICAO) > 0 THEN 'SIM'
----WHEN CHARINDEX('PIPA', TipoEquipamento.Descricao) > 0 THEN 'SIM'
