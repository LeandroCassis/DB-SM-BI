...
GO
CREATE VIEW [dbo].[WH_COTAÇÕES] AS
...
SELECT
    ...
    -- ORDEM DE COMPRA
    E410COT.FILOCP 'COD FILIAL OC',
    E405SOL.FILSOL 'COD FILIAL SOLICITAÇÃO',
...
--WHERE  YEAR(E410COT.DATCOT) >=2024
...
FROM [SQLMML].[Sapiens_Prod].[dbo].[E410COT] WITH (NOLOCK)                        -- Tabela principal: Cotações
    ...
    OUTER APPLY (
        SELECT TOP 1 IPO.CODEMP,  IPO.CODFIL, IPO.NUMOCP, IPO.SEQIPO
        FROM [SQLMML].[Sapiens_Prod].[dbo].[E420IPO] IPO WITH (NOLOCK)
        WHERE E420OCP.CODEMP = IPO.CODEMP
        AND E420OCP.CODFIL = IPO.CODFIL
        AND E420OCP.NUMOCP = IPO.NUMOCP
        AND E410COT.CODPRO = IPO.CODPRO
 --       AND E410COT.PROSER <> 'S'
   ...
   -- ITENS DA NOTA FISCAL DE COMPRA PRODUTO
    ...
    OUTER APPLY (
        SELECT TOP 1 ISO.CODEMP,  ISO.CODFIL, ISO.NUMOCP, ISO.CODSER, ISO.SEQISO
        FROM [SQLMML].[Sapiens_Prod].[dbo].[E420ISO] ISO WITH (NOLOCK)
        WHERE E420OCP.CODEMP = ISO.CODEMP
        AND E420OCP.CODFIL = ISO.CODFIL
        AND E420OCP.NUMOCP = ISO.NUMOCP
        AND E410COT.CODSER = ISO.CODSER
...
WHERE  YEAR(E410COT.DATCOT) >=2020
GO